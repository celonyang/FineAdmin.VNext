...
namespace FineAdmin.Service
{
    public abstract class BaseService<T> where T : class, new()
    {
        ...
        public dynamic GetListByFilter(T filter, PageInfo pageInfo, string where = null)
        {
            ...
            int total = 0;
            var list = BaseRepository.GetByPage(new SearchFilter
            {
                PageIndex = pageInfo.page,
                PageSize = pageInfo.limit,
                ReturnFields = pageInfo.returnFields,
                Param = filter,
                Where = where,
                OrderBy = _orderBy
            }, out total);
        ...
        public dynamic GetPageUnite(T filter, PageInfo pageInfo, string where = null)
        {
            ...
            int total = 0;
            var list = BaseRepository.GetByPageUnite(new SearchFilter
            {
                Prefix = pageInfo.prefix,
                PageIndex = pageInfo.page,
                PageSize = pageInfo.limit,
                ReturnFields = pageInfo.returnFields,
                Param = filter,
            ...
            if (!string.IsNullOrEmpty(StartEndDate) && StartEndDate != " ~ ")
            {
                if (StartEndDate.Contains("~"))
                {
                    ...
                    if (!string.IsNullOrEmpty(start))
                    {
                        ...
                        {
                            _where += string.Format(" and {1}CreateTime>='{0} 00:00:00'", start, prefix);
                        }
                        else
                        {
                            _where += string.Format(" and CreateTime>='{0} 00:00:00'", start);
                        }
                    }
                    if (!string.IsNullOrEmpty(end))
                    {
                        if (!string.IsNullOrEmpty(prefix))
                        {
                            _where += string.Format(" and {1}CreateTime<='{0} 00:59:59'", end, prefix);
                        }
                        else
                        {
                            _where += string.Format(" and CreateTime<='{0} 00:59:59'", end);
                        }
                    }
                }
            }
